...
- Silero VAD 语音活动检测 + SenseVoice 中文转写（asr.py 可插拔：SenseVoice 默认/whisper 回退，2026-09-03）
...
15. **流畅性快赢：帧率分档 + VAD 缩短 + 受理气泡显原文（2026-09-03，调研驱动）**：①skin 动画帧率随状态分档（idle 8fps / thinking·talking 25fps，相位按毫秒等比推进保节奏）——动整窗的分层窗口每帧触发 DWM 全窗 alpha 重合成，idle 降帧即省 CPU；②VAD 静音判完 1.5s→1.0s（行业典型 0.7~1.0s；截断由 consumer 合并连续片段兜底，内容不丢）；③受理气泡显示转写原文（60s 兜底、回复覆盖）——"听对没"一眼可辨，感知延迟直降。来源：ElevenLabs voice-agent-latency-optimization / arXiv 2508.04721 / Qt 官方文档。
16. **config 三文件分层 + 记忆阈值配置化（2026-09-04，方案 A 经用户确认）**：`config/common.toml`（BACKEND 选择器 + 共享键：判定器/TTS/ASR/SPRITE/MEMORY_*）+ `config/openai.toml` + `config/astrbot.toml`（后端专属）；合并规则 = common 读 BACKEND → 加载对应文件 → 后端键覆盖同名共享键。`load_config` 迁出 skin.py 到新 `config_loader.py`（配置加载是装配层职责）；缺文件抛 `ConfigError` 带修复指引。记忆阈值默认值移到策略 owner（`backends/openai.py` 的 `DEFAULT_*`），经 `MEMORY_MAX_TURNS`/`MEMORY_KEEP_RECENT`/`MEMORY_MAX_FACTS` 配置键覆盖；memory.py 回归纯机制（删除自用为零的常量）。`create_backend` 的 openai 分支改为显式传 config（原为裸构造让后端自己偷读文件）。判定器兜底密钥（本地端点任意串）放 common，openai 模式被真实 key 覆盖。
17. **形象动画：PSD 直驱渲染器（2026-09-04，路线经用户确认）**：不做 Cubism 手动绑定（GUI 创作无法代做），改自研 `psd_renderer.py` 吃 See-through 拆层 PSD。链路：`tools/prepare_psd.py`（左右分离列间隙法/边缘扩展 roll 传播/导 manifest+层 PNG）→ `PsdRenderer(QWidget)`（眨眼 L/R 独立随机/瞳孔追鼠标/口型开合/呼吸/发摆/头组绕颈点摆）→ skin 按 `SPRITE` 后缀装配（.json=分层渲染、.gif=QMovie、其余=静态图）。分层模式下窗口恒定不动、只在控件内重绘（顺带落实 2026-09-03 调研的 DWM 友好路径）。真 Live2D 留作升级：同一份拆层 PSD 可进 Cubism，渲染器缝可换。已知坑：未 show 控件上调 `mapToGlobal` 是原生崩溃（渲染器已加 isVisible 守卫）；`grab()` 对半透明未显示控件在本机崩，离屏验证用 `render(QPainter)`。
18. **AstrBot 适配插件：风格注入 + 工具调用统一链（2026-09-09）**：`astrbot_plugin_deskfriend/`（拷至 AstrBot data/plugins 即装）。①按 OneBot **self_id**（非 sender——桌宠 USER_ID 即主人 QQ，会撞）识别桌宠来源，`on_llm_request` 追加"简短聊天/禁动作描写/禁 markdown"约束（追加不替换），其它来源零侵入；②`look_at_screen` FunctionTool 经 OneBot action `deskfriend_tool` → 桥 tool_handler → 后端 tool_sink → **`spine.execute_tool` 唯一分发点**（重构：原 tool_executer 拆为 execute_tool + openai 打包 wrapper，astrbot 后端不再自持执行逻辑）→ 截图 data-url 回包 → 插件经 `FunctionTool.call(context: ContextWrapper).messages` **同轮注入**（`mark_as_temp` 不落持久历史——v4.27 源码核实的官方数据面）。旧 `[look_at_screen]` 文本标记保留作回退（插件未加载时兜底）。API 全部对本机 v4.27.2 源码核对并用 SDK 4.28 导入验证；真机（家部署）调试项见当日日志。
